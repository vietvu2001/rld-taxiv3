...
def worker(num, actor, critic, env, artf=False):
    ...
    while True:  # Run until solved
        ...
        #while True:
            ...
            running_reward = sum(books) / len(books)
...
def evaluate_branch(actor, critic, env, artf=False):  # greedy
    if artf:
        env.reset()
        env.current = env.encode((0, 2, 0, 1))
        state = env.current
    else:
        state = env.reset()
    actions = []
...
B = 3  # budget = 3
num_walls = np.random.randint(0, 3)
num_special = B - num_walls

for i in range(num_walls):
    env = env.transition()

cells = random.sample(random_cells, num_special)
for cell in cells:
    env.special.append(cell)

print(num_walls)
with open("my_text_random_2", "w") as file:
    for wall in first_walls:
        if wall not in env.walls:
            file.write("wall: ")
            file.write(str(wall))
            file.write("\n")
    for cell in env.special:
        file.write("special cell: ")
        file.write(str(cell))
        file.write("\n")
...
actor.save("my_actor_random_2")
critic.save("my_critic_random_2")'''
first_walls = env.walls
env = env.transition([(4, 2), (2, 4)])
env.special.append((2, 1))
env.special.append((2, 2))

with open("my_text_freq_3", "w") as file:
    for wall in first_walls:
        if wall not in env.walls:
            file.write("wall: ")
            file.write(str(wall))
...
actor.save("my_actor_freq_3")
critic.save("my_critic_freq_3")