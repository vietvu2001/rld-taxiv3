...
class Heap():
    def __init__(self, model, data,size):
        ...
        for element in data:
            element = np.reshape(element, (1, 12))
...
h = Heap(model, x_test[0 : 32], 32)
h.build_heap()

def get_ordered_sequence(list):
    N = len(list)
    index_1 = random.randint(0, N - 4)
    index_2 = random.randint(index_1 + 1, N - 3)
    index_3 = random.randint(index_2 + 1, N - 2)
    index_4 = random.randint(index_3 + 1, N - 1)
    return [list[index_1], list[index_2], list[index_3], list[index_4]]


for i in range(25000):
    #seq = random.sample(modifications, k=4)
    seq = get_ordered_sequence(modifications)
    seq = np.array(seq)
    seq = np.reshape(seq, (1, 12))
...
with open("sl_result_4.txt", "w") as file:
